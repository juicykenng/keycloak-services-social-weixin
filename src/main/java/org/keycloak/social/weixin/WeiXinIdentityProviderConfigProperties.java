...
final class WeiXinIdentityProviderConfigProperties {

    private WeiXinIdentityProviderConfigProperties() {
    }

    static List<ProviderConfigProperty> build() {
        return ProviderConfigurationBuilder.create()
                ...
                .type(ProviderConfigProperty.STRING_TYPE)
                .add()
                .property().name(WeiXinIdentityProvider.CUSTOMIZED_CALLBACK_URL)
                .label("OAuth 回调 URL")
                .helpText("覆盖发送给微信的 redirect_uri。留空时使用 Keycloak 默认地址：/realms/{realm}/broker/{alias}/endpoint")
                .type(ProviderConfigProperty.STRING_TYPE)
                .add()
